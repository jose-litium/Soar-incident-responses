...
Below is a sample JSON payload representing a security incident. This format is used when interacting with the SOAR webhook (for example, using `curl`, Google Chronicle, or other automated sources). Ensure you append your configured `WEBHOOK_TOKEN` to the URL via the `?token=` parameter:
